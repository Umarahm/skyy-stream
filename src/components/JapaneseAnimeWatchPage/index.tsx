...
const mapMiruroWatchToSources = (
  ...
  if (providerName === "moo") {
    const hlsFirst = usableStreams.filter(
      (item: any) => String(item?.type || "").toLowerCase() === "hls",
    );
    if (hlsFirst.length > 0) {
      const preferred1080 = hlsFirst.filter(
        (item: any) => String(item?.quality || "") === "1080p",
      );
...
const normalizeSkipType = (
  value: any,
): "opening" | "closing" | "recap" | null => {
  const lowered = String(value || "").toLowerCase();
  if (lowered === "op" || lowered.includes("open") || lowered.includes("intro"))
    return "opening";
  if (
    lowered === "ed" ||
    lowered.includes("end") ||
    lowered.includes("clos") ||
    lowered.includes("outro")
  ) {
    return "closing";
  }
...
const extractSkipSegments = (
  ...
  const normalized = rawEntries
    .map((entry) => {
      const start = Number(
        entry?.start ?? entry?.startTime ?? entry?.from ?? entry?.begin,
      );
      const end = Number(
        entry?.end ?? entry?.endTime ?? entry?.to ?? entry?.finish,
      );
      const type = normalizeSkipType(
        entry?.type ?? entry?.skipType ?? entry?.segmentType ?? entry?.name,
      );
      if (
        ...
        label:
          type === "opening"
            ...
            : type === "closing"
              ...
              : "Skip Recap",
        provider: String(
          entry?.provider || entry?.source || entry?.server || "",
        ).toLowerCase(),
        episodeNo: Number(entry?.episode ?? entry?.episodeNumber ?? 0),
      };
    })
    .filter(Boolean) as Array<
    SkipSegment & { provider: string; episodeNo: number }
  >;

  const exactEpisodeMatches = normalized.filter(
    (segment) => segment.episodeNo === episode,
  );
  const genericEpisodeMatches = normalized.filter(
    (segment) => segment.episodeNo <= 0,
  );
...
const toMediaCards = (
  payload: any,
  kind: "relations" | "recommendations",
): MediaCard[] => {
  const source =
    payload?.[kind] ||
    payload?.data?.[kind] ||
  ...
  return list
    .map((item: any) => {
      const media =
        item?.node || item?.mediaRecommendation || item?.media || item;
      const id = media?.id;
      if (!id) return null;
      const title =
        media?.title?.english ||
...
const JapaneseAnimeWatchPage = () => {
  ...
  const [selectedProvider, setSelectedProvider] = useState(
    String(provider || "").toLowerCase() || "kiwi",
  );
  ...
  const availableMiruroProviders = useMemo(() => {
    return Object.entries(providerEpisodeLookup)
      .filter(
        ([providerName]) =>
          !HIDDEN_PROVIDERS.has(String(providerName || "").toLowerCase()),
      )
  ...
  useEffect(() => {
    const loadMeta = async () => {
      ...
      try {
        ...
        setRecommendations(
          recommendationsPayload?.recommendations?.slice(0, 5) ||
            recommendationsPayload?.nodes?.slice(0, 5) ||
            [],
        );
      } catch (error) {
        console.error(error);
  ...
  useEffect(() => {
    ...
    const preferredDefault = availableMiruroProviders.includes("kiwi")
      ? "kiwi"
    ...
    if (
      !isExplicitAnikoto &&
      (!selectedProvider ||
        !availableMiruroProviders.includes(selectedProvider))
    ) {
      if (preferredDefault) {
  ...
  const miruroAvailableTypes = useMemo((): ("sub" | "dub")[] => {
    ...
    const hasSub =
      Boolean(lookup.sub) ||
      Boolean(deriveEpisodeIdForProvider(episodeId, selectedProvider, "sub"));
  ...
  useEffect(() => {
    const loadEpisodes = async () => {
      ...
      try {
        ...
        setProviderEpisodeLookup(
          getEpisodeIdLookupForEpisode(payload, episode),
        );
      } catch (error) {
        console.error(error);
        setProviderEpisodeLookup({});
      ...
      try {
        ...
        if (isMiruroFlow) {
          ...
          const providerFallbackOrder = Array.from(
            new Set([
              normalizedProvider,
              ...preferredFallback.filter((name) =>
                availableMiruroProviders.includes(name),
              ),
              ...availableMiruroProviders,
            ]),
          ).filter(Boolean);
          for (const candidateProvider of providerFallbackOrder) {
            ...
            const miruroWatch =
              await getMiruroWatchByEpisodeId(lookupEpisodeId);
            const mappedSources = mapMiruroWatchToSources(
              miruroWatch,
              candidateProvider,
              selectedType,
            );
  ...
  const anikotoSourceTypes: ("sub" | "dub")[] = Array.from(
    new Set(
      sources.map((s: any) => normalizeAudioType(s?.type)).filter(Boolean),
    ),
  ) as ("sub" | "dub")[];
  ...
  const filteredSources = useMemo(
    () =>
      sources.filter(
        (source: any) => normalizeAudioType(source?.type) === selectedType,
      ),
  ...
  const skipSegments = useMemo(
    () =>
      extractSkipSegments(
        [watchData, watchData?.providerPayload, selectedSource?.rawStream],
        selectedProvider,
        episode,
      ),
  ...
  const animeName = useMemo(() => {
    ...
    const firstFound = candidates.find(
  ...
  const backHref = animeId
    ? `/anime-details?id=${animeId}`
    : slug
      ? `/anime-detail?slug=${slug}`
      : "/anime";
  const providerMeta = [selectedProvider, mode, subType]
    .filter(Boolean)
    .join(" • ");
        ...
        <main className={styles.main}>
          <div className={styles.mainInner}>
            <section className={styles.playerSection}>
              {loading ? (
                <p className={styles.empty}>Loading watch sources...</p>
              ) : null}
              {!loading && !selectedSource?.proxyUrl ? (
                <p className={styles.empty}>
                  No source found for this episode.
                </p>
              ) : null}
              {!loading && selectedSource?.proxyUrl ? (
